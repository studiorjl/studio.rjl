...
function analytics() {
  return `
    <script>
      (function () {
        try {
          if (/[?&]analytics-on(=|&|$)/.test(location.search)) { localStorage.removeItem("rjl-analytics-off"); }
          else if (/[?&]analytics-off(=|&|$)/.test(location.search)) { localStorage.setItem("rjl-analytics-off", "1"); }
          if (localStorage.getItem("rjl-analytics-off") === "1") { window["ga-disable-${site.googleAnalyticsId}"] = true; }
        } catch (e) {}
      })();
    </script>
